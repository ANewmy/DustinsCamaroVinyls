...
app.get('/adminIndex',function (req,res) {
  if(req.session.user)
    res.sendFile(`${publicPath}/adminIndex.html`)
  else
    res.sendFile(`${publicPath}/index.html`)
})
...
app.get("/stores", function(req,res){
  if(req.session.user)
    res.sendFile(`${publicPath}/adminStore.html`)
  else
    res.sendFile(`${publicPath}/adminLogin.html`)
})
...
app.post("/updateStore",function(req,res){
  ...
  var data=req.body
  var query={_id: ObjectId(data._id)}
  var update={$set:{vinylName:data.vinylName,
    price:data.price,
...
{
  console.log(query)
  db.collection("vinylStore").find(query).toArray(function (err,results) {
